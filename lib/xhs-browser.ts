...
export type XhsBrowserResult =
  | { ok: true; data: XhsBrowserData }
    ...
    if (jsonMatches) {
      for (const jsonStr of jsonMatches) {
        try {
          ...
          // 提取 URL
          const urlMatches = objStr.match(/https?:\/\/[^"'\s]+\.(jpg|jpeg|png|webp|gif)/gi)
          if (urlMatches) {
...
// 获取网页内容（浏览器模拟）
async function fetchHtmlBrowserMode(url: string): Promise<string> {
  console.log('开始获取网页内容（浏览器模拟模式）:', url)
...
// 主要函数：浏览器模拟爬取小红书数据
export async function fetchXhsBrowserData(noteUrl: string): Promise<XhsBrowserResult> {
  console.log('使用浏览器模拟方案获取小红书数据:', noteUrl)
  ...
  try {
    ...
    const html = await fetchHtmlBrowserMode(noteUrl)
