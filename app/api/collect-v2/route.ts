...
// 分析图片
async function analyzeImageWithQwenVL(imageUrl: string): Promise<any> {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!process.env.DASHSCOPE_API_KEY) {
      throw new Error('关键错误：.env.local 中的 DASHSCOPE_API_KEY 未能加载，请检查文件是否在根目录并重启 npm run dev');
    }
    ...
    try {
      analysisResult = await analyzeImageWithQwenVL(imageData)
...
export async function GET() {
  const response = NextResponse.json({
    success: true,
    message: '请通过 POST /api/collect-v2 提交数据',
  })
  return addCorsHeaders(response)
}
